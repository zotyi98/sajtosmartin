import { getAllCatalogUpgrades } from './upgradeCatalog.js';

/** Áremelkedés minden vásárlásnál (Cookie Clicker ~1.15) */
export const BUILDING_PRICE_GROWTH = 1.17;
...
export const defaultUpgrades = [
    ...
    { id: 17, icon: "🏢", name: "Irodaház", desc: "+6,000 pont / mp", cost: 2500000, power: 6000, type: "bps", owned: 0 },
    { id: 4, icon: "🖥️", name: "Gépújraindító", desc: "+28,000 pont / mp", cost: 15000000, power: 28000, type: "bps", owned: 0 },
    { id: 5, icon: "🌀", name: "Segédpropeller", desc: "+130,000 pont / mp", cost: 80000000, power: 130000, type: "bps", owned: 0 },
    { id: 11, icon: "🍻", name: "Garrison tavern", desc: "+600,000 pont / mp", cost: 400000000, power: 600000, type: "bps", owned: 0 },
    { id: 18, icon: "🎰", name: "Kaszinó", desc: "+2,800,000 pont / mp", cost: 2500000000, power: 2800000, type: "bps", owned: 0 },
    { id: 6, icon: "🧀", name: "Sajtgyár", desc: "+14,000,000 pont / mp", cost: 12000000000, power: 14000000, type: "bps", owned: 0 },
    { id: 8, icon: "🚗", name: "Autóbiznisz", desc: "+70,000,000 pont / mp", cost: 60000000000, power: 70000000, type: "bps", owned: 0 },
    { id: 12, icon: "🏰", name: "Roxfort", desc: "+400,000,000 pont / mp", cost: 350000000000, power: 400000000, type: "bps", owned: 0 },
    { id: 20, icon: "🏭", name: "Nano-Gyár", desc: "+2,000,000,000 pont / mp", cost: 1800000000000, power: 2000000000, type: "bps", owned: 0 },
    { id: 7, icon: "❤️", name: "Eszter támogatása", desc: "DUPLA BPS és Kattintás! (1. prestige után)", cost: 8000000000000, power: 0, type: "special", owned: 0 },
    { id: 13, icon: "👁️", name: "Mordor", desc: "+8,000,000,000 pont / mp", cost: 45000000000000, power: 8000000000, type: "bps", owned: 0 },
    { id: 21, icon: "🚀", name: "Űrállomás", desc: "+40,000,000,000 pont / mp (1. prestige)", cost: 280000000000000, power: 40000000000, type: "bps", owned: 0 },
    { id: 22, icon: "⏱️", name: "Időgép", desc: "+200,000,000,000 pont / mp (2. prestige)", cost: 1500000000000000, power: 200000000000, type: "bps", owned: 0 },
    { id: 23, icon: "🌌", name: "Multiverzum Kapu", desc: "+1e12 / mp (3. prestige, tier kell!)", cost: 80000000000000000, power: 1000000000000, type: "bps", owned: 0 },
    { id: 24, icon: "⚛️", name: "Kvantum Depó", desc: "+5e12 / mp (4. prestige)", cost: 4e17, power: 5e12, type: "bps", owned: 0 },
    { id: 25, icon: "🌙", name: "Holdbázis", desc: "+2.5e13 / mp (5. prestige)", cost: 2e19, power: 2.5e13, type: "bps", owned: 0 },
    { id: 26, icon: "🌀", name: "Dimenziókapu", desc: "+1.2e14 / mp (6. prestige)", cost: 1e21, power: 1.2e14, type: "bps", owned: 0 },
    { id: 27, icon: "♾️", name: "Örök Bicikli", desc: "+6e14 / mp (7. prestige)", cost: 5e22, power: 6e14, type: "bps", owned: 0 },
    { id: 28, icon: "💎", name: "Martin Szíve", desc: "+3e15 / mp (8. prestige)", cost: 2e24, power: 3e15, type: "bps", owned: 0 }
];

/** Cookie Clicker: 5 tier / épület — küszöbök és hatások (balanszolt) */
export const BUILDING_TIER_THRESHOLDS = [1, 10, 50, 100, 200];
export const BUILDING_TIER_NAMES = ['Indítás', 'Csapat', 'Hálózat', 'Birodalom', 'Legenda'];
export const BUILDING_TIER_EFFECT = [0.05, 0.07, 0.09, 0.11, 0.13];
export const BUILDING_TIER_COST_MULT = [0.7, 3.5, 18, 85, 380];

export function getBuildingTierUpgradesList() {
    const list = [];
    defaultUpgrades.forEach((b) => {
        ...
        for (let t = 0; t < 5; t++) {
            list.push({
                ...
                buildingId: b.id,
                tier: t,
                threshold: BUILDING_TIER_THRESHOLDS[t],
                tierName: BUILDING_TIER_NAMES[t],
                buildingName: b.name,
                icon: b.icon,
                effect: BUILDING_TIER_EFFECT[t],
                costMult: BUILDING_TIER_COST_MULT[t]
            });
        }
    });
    return list;
...
export const buildingTierUpgrades = getBuildingTierUpgradesList();

const baseExtraUpgrades = [
    ...
    { id: 110, name: "Szauron Szeme", desc: "Mordor x1.5 BPS", cost: 50000000000000, reqBuilding: 13, reqCount: 5, targetId: 13, mult: 1.5 },
    { id: 122, name: "Műholdas Adatátvi tel", desc: "Űrallómás x1.5 BPS", cost: 250000000000000, reqBuilding: 21, reqCount: 12, targetId: 21, mult: 1.5 },
    { id: 123, name: "Paradoxon Pájzs", desc: "Időgép x1.5 BPS", cost: 1200000000000000, reqBuilding: 22, reqCount: 12, targetId: 22, mult: 1.5 },
    { id: 124, name: "Kvantum hűtés", desc: "Kvantum Depó x1.5 BPS", cost: 8e18, reqBuilding: 24, reqCount: 15, targetId: 24, mult: 1.5 },
    { id: 125, name: "Holdi gravitáció", desc: "Holdbázis x1.5 BPS", cost: 4e20, reqBuilding: 25, reqCount: 15, targetId: 25, mult: 1.5 },
    { id: 126, name: "Dimenzió stabilizátor", desc: "Dimenziókapu x1.5 BPS", cost: 2e22, reqBuilding: 26, reqCount: 12, targetId: 26, mult: 1.5 }
];

/** Alap + generált milestone fejlesztések (~180 db) */
export const extraUpgradesData = [...baseExtraUpgrades, ...getAllCatalogUpgrades()];

export const prestigeSkillsData = [
    { id: 301, name: "Isteni Láncolaj", desc: "Minden BPS és Kattintás +22% szintenként (max 3).", baseCost: 5, repeatable: true, maxLevel: 3, req: null, x: 50, y: 15 },
    { id: 302, name: "Gyémánt Láncolaj", desc: "További +22% BPS és kattintás szintenként (max 3).", baseCost: 18, repeatable: true, maxLevel: 3, req: 301, x: 50, y: 30 },
    ...
    { id: 304, name: "Küllő-Mágnes", desc: "Küllőnként 0,25% passzív bónusz (max ~38%).", baseCost: 28, repeatable: false, req: 303, x: 50, y: 60 },
    { id: 404, name: "Sötét Anyag Küllő", desc: "+1,8% globális termelés / szint (max 15).", baseCost: 70, repeatable: true, maxLevel: 15, req: 304, x: 50, y: 82 },
    ...
    { id: 205, name: "Villámkezű", desc: "A kattintásod megkapja a BPS 0,12%-át extra erőként.", baseCost: 22, repeatable: false, req: 202, x: 88, y: 45 },
...
export const achievements = [
    ...
    { id: 'meta_ascension', icon: '🌠', name: 'Mennyei lépcső', desc: 'Első mennyei emelkedés.', check: { type: 'ascensionCount', value: 1 }, reward: { type: 'goldenSpokes', amount: 2 }, done: false },
