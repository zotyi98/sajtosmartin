...
window.updateUI = function() {
    ...
    const cosmeticsList = document.getElementById('cosmetics-section');
    if (cosmeticsList) {
        const hasCheese = GameState.cosmetics && GameState.cosmetics.includes('cheese_cursor');
        cosmeticsList.innerHTML = `
            <div class="extra-upgrade-item ${hasCheese ? 'owned' : (GameState.bikes >= 50000 ? 'affordable' : 'disabled')}">
                <b>Sajtos kurzor</b><br>
                <span style="color:#78909c;">Változtasd sajttá az egeredet! Egyedi kurzor élmény.</span><br>
                <b style="color:#d32f2f; font-family:'Bangers'; font-size:16px;">50000 🚲</b>
                <button onclick="window.buyCosmetic('cheese_cursor')" style="margin-top:8px; width:100%;" ${hasCheese || GameState.bikes < 50000 ? 'disabled' : ''}>${hasCheese ? 'Már megvan' : 'Megvásárlás'}</button>
            </div>
        `;
    }
...
window.buyCosmetic = function(id) {
    ...
    window.updateUI(); saveUserProgress();
};
...
async function loadUserProgressFromDB() {
    ...
    let parsed = (firebaseData && localData) ? ((firebaseSaved > localSaved) ? firebaseData : localData) : (firebaseData || localData);
    ...
    window.updateInventoryUI(); window.recalculateStats(); window.updateUI(); window.applyCosmetics();
}
...
window.login = async function() {
    ...
    // 💾 Jelszó mentése, ha a checkbox be van jelölve
...
// 💾 Elmentett jelszó betöltése az oldal betöltésekor
...
// Az oldal betöltésekor automatikusan betöltjük az elmentett jelszót
window.addEventListener('load', () => {
    window.loadRememberedPassword();
});
